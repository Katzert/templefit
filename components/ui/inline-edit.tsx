  ...
  if (isEditing && !disabled) {
    if (multiline) {
      return (
        <textarea
          ...
          className={cn(
            "w-full bg-black/5 dark:bg-black/40 border border-temple-gold/50 rounded-lg p-2 text-white focus:outline-none focus:ring-1 focus:ring-temple-gold resize-none",
            className
          )}
          rows={3}
        />
    ...
    return (
      <input aria-label="Editar valor inline" ref={inputRef as React.RefObject<HTMLInputElement>}
        ...
        placeholder={placeholder}
        className={cn(
          "w-full bg-black/5 dark:bg-black/40 border border-temple-gold/50 rounded-lg p-2 text-white focus:outline-none focus:ring-1 focus:ring-temple-gold",
          className
        )}
      />
    );
